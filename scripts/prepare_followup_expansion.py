#!/usr/bin/env python3
"""Initialize follow-up shards after the first expansion has completed."""
...
CACHE = ROOT / "discovery" / "all_papers_cache_320000.json"
EXISTING_SHARDS = range(16)
NEW_SHARDS = range(16, 24)
PIPELINE_VERSION = 7
...
def main() -> None:
    ...
    if unmatched:
        ...
        raise RuntimeError(
            f"Could not match {len(unmatched)} outputs to the expanded cache: {preview}"
        )
    ...
    summary = {
        ...
        "existing_json_count": len(existing_stems),
        "matched_existing_ids": len(existing_ids),
        "new_shards": list(NEW_SHARDS),
    }
    with (ROOT / "discovery" / "followup_preparation.json").open("w") as handle:
